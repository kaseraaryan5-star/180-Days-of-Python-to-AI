...
# # 2D PLOT
stocks = pd.read_csv('/Users/aryankasera/Desktop/180-Days-Python/06_Data_Visualisation/weekly_stocks.csv')
print(stocks.head())

#line plot
print(stocks['MSFT'].plot(kind='line'))
print(plt.show())
print(stocks.plot(kind='line',x='Date'))
print(plt.show())


#bar chart -> single -> horizontal -> multiple
#using tips
temp = pd.read_csv('/Users/aryankasera/Desktop/180-Days-Python/06_Data_Visualisation/batsman_season_record.csv')
print(temp.head())

print(temp['2015'].plot(kind='bar'))
print(plt.show())

print(tips.groupby('sex')['total_bill'].mean().plot(kind='bar'))
print(plt.show())

#multiple bar chart
print(temp.plot(kind='bar'))
print(plt.show())

#stacked chart
print(temp.plot(kind='bar',stacked=True))
print(plt.show())


#HISTOGRAM
#using stocks
print(stocks.plot(kind='hist'))
print(plt.show())



# PIE CHART -> single and multiple
df = pd.DataFrame(
    {   
...
print(df.head())
print(df['match1'].plot(kind='pie',labels=df['batsman'].values,autopct='%0.1f%%'))
print(plt.show())

#multiple pie charts
print(df[['match1','match2','match3']].plot(kind='pie',subplots=True,figsize=(15,8)))
print(plt.show())


#multiple separate graphs together
#using stocks
print(stocks.plot(kind='line',subplots=True))
print(plt.show())

#on multiindex dataframe
#using tips
print(tips.pivot_table(index=['day','time'],columns=['sex','smoker'],values='total_bill',aggfunc='mean').plot(kind='bar'))
print(plt.show())

print(tips.pivot_table(index=['day','time'],columns=['sex','smoker'],values='total_bill',aggfunc='mean').plot(kind='pie',subplots=True,figsize=(20,10)))
print(plt.show())